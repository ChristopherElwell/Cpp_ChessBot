    ...
    void run_perft_test(int max_draft)
    {
        ...
        for (auto perft_test : PERFT_DATA)
        {
            ...
            for (int i = 0; i < max_draft; i++)
            {
                ...
                {
                    std::cout << "\tPassed, Draft: " << i + 1 << " | Correct Perft: " << perft << "\n";
                    drafts_passed++;
                }
                else
                {
                    std::cout << "\tFailed, Draft: " << i + 1 << " | Correct Perft: " << perft_test.second[i] << " | This Perft: " << perft << "\n";
                }
            }
...
namespace test_positions
{
    ...
    {
        std::vector<std::vector<std::string>> pzls = helpers::read_csv(priv::WIN_AT_CHESS_FILE);
        int i = 0;
        int passed = 0;
        ...
        for (auto pzl : pzls)
        {
            ...
            std::cout << "\n\nRUNNING TESTS: " << pzl[2] << "\n"
                      << pzl[0] << "\n";
            std::string bot_move = control::get_bot_move_at_depth_algebreic(pzl[0], 7);
            if (bot_move == pzl[1])
            {
                std::cout << "\nPASSED [" << pzl[1] << "]\n";
                passed++;
            ...
            {
                std::cout << "\nFAILED | Bot Move: [" << bot_move << "] Correct Move: [" << pzl[1] << "]\n";
            }
        }
